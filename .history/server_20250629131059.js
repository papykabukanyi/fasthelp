...
app.get('/health', (req, res) => {
    ...
    const response = {
        ...
        railway: {
            url: 'https://fasthelp-production.up.railway.app/',
            host: req.get('Host'),
            port: PORT
        }
    };
...
// Request logging middleware - VERY FIRST
app.use((req, res, next) => {
    const timestamp = new Date().toISOString();
    console.log(`📥 REQUEST: ${req.method} ${req.path} at ${timestamp}`);
    console.log(`   - IP: ${req.ip || req.connection.remoteAddress}`);
    console.log(`   - Host: ${req.get('Host')}`);
    console.log(`   - User-Agent: ${req.get('User-Agent')}`);
    next();
...
async function connectRedis() {
    try {
        ...
        log('error', 'Redis connection error - server will continue without Redis', { error: err.message });
        redisClient = null;
        return false;
    }
}
