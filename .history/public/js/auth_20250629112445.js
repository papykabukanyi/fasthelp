...
class AuthManager {
    ...
    checkAuthStatus() {
        ...
        if (token) {
            ...
            if (window.location.pathname.includes('donor-signup.html')) {
        ...
        try {
            ...
            if (error.message && error.message.includes('pending approval')) {
                FastHelpUtils.showNotification(
                    '⏳ Your account is still pending approval. Please wait for an admin to approve your account. You will receive an email once approved!', 
                    'warning',
                    5000
                );
            } else {
