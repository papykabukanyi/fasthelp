...
class AuthManager {
    ...
    async verifyToken(token) {
        try {
            ...
            if (response.ok) {
                ...
                this.handleAuthSuccess(userData, token);
            } else {
    ...
    handleAuthSuccess(userData, token) {
        ...
        if (window.location.pathname === '/donor-signup') {
        ...
        try {
            ...
            if (error.message && error.message.includes('pending approval')) {
                FastHelpUtils.showNotification(
                    '⏳ Your account is pending approval. Please wait for an admin to approve your account. You\'ll receive an email notification once approved!', 
                    'warning',
