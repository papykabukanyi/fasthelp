...
const EmailTemplateHelper = require('./email-template-helper');
        ...
        const html = `
            <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; background: #f8fafc; padding: 20px;">
                <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 30px; border-radius: 12px; text-align: center; margin-bottom: 20px;">
                    <h1 style="margin: 0; font-size: 24px;">🍽️ New Donation Available!</h1>
                    <p style="margin: 10px 0 0 0; opacity: 0.9;">Help your Austin community</p>
                </div>
                
                <div style="background: white; padding: 30px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                    <h2 style="color: #1f2937; margin-top: 0;">${donation.title}</h2>
                    <p style="color: #6b7280; font-size: 16px; line-height: 1.6;">${donation.description}</p>
                    
                    <div style="background: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
                        <p style="margin: 0; color: #374151;"><strong>📍 Location:</strong> ${donation.address || 'Austin, TX'}</p>
                        <p style="margin: 10px 0 0 0; color: #374151;"><strong>🏷️ Type:</strong> ${donation.type}</p>
                        ${donation.dropoffInstructions ? `<p style="margin: 10px 0 0 0; color: #374151;"><strong>📝 Instructions:</strong> ${donation.dropoffInstructions}</p>` : ''}
                    </div>
                    ...
                    <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb; text-align: center;">
                        <p style="color: #9ca3af; font-size: 14px; margin: 0;">
                            💝 Thanks for helping make Austin more caring! 
                            <br>
                            <a href="${process.env.BASE_URL || 'http://localhost:3000'}/unsubscribe" style="color: #6b7280;">Unsubscribe</a>
                        </p>
                    </div>
                </div>
            </div>
...
app.post('/api/donations', authenticateToken, upload.single('image'), async (req, res) => {
    try {
        ...
        await sendDonationNotificationEmails(donation);
...
app.post('/api/donations/:id/pickup', async (req, res) => {
    try {
        ...
        const confirmationHtml = `
            <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; background: #f8fafc; padding: 20px;">
                <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 30px; border-radius: 12px; text-align: center; margin-bottom: 20px;">
                    <h1 style="margin: 0; font-size: 24px;">🎉 Thank You for Your Kindness!</h1>
                    <p style="margin: 10px 0 0 0; opacity: 0.9;">You're making Austin more caring</p>
                </div>
                
                <div style="background: white; padding: 30px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                    <h2 style="color: #1f2937; margin-top: 0;">Pickup Confirmed! 💝</h2>
                    <p style="color: #6b7280; font-size: 16px; line-height: 1.6;">Dear ${pickerName},</p>
                    <p style="color: #6b7280; font-size: 16px; line-height: 1.6;">
                        Amazing! You've successfully picked up: <strong style="color: #1f2937;">${donation.title}</strong>
                    </p>
                    
                    <div style="background: #f0f9ff; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #0ea5e9;">
                        <p style="margin: 0; color: #0f172a; font-size: 16px;">
                            <strong>🌟 You're a community hero!</strong> By picking up this donation, you're helping connect resources with people who need them most. Every act of kindness creates a ripple effect that makes Austin stronger.
                        </p>
                    </div>
                    
                    <p style="color: #6b7280; font-size: 16px; line-height: 1.6;">
                        <strong>Next step:</strong> Please let the community know where you delivered this item so others can see the impact! This helps build trust and shows how our network of caring works.
                    </p>
                    
                    <div style="text-align: center; margin: 30px 0;">
                        <a href="${req.protocol}://${req.get('host')}/delivery-confirmation?tracking=${tracking._id}" 
                           style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                                  color: white; 
                                  text-decoration: none; 
                                  padding: 15px 30px; 
                                  ...
                                  display: inline-block; 
                                  transition: transform 0.2s;">
                            📍 Share Where You Delivered
                        </a>
                    </div>
                    
                    <div style="background: #fef3c7; padding: 15px; border-radius: 8px; margin: 20px 0;">
                        <p style="margin: 0; color: #92400e; font-size: 14px;">
                            💡 <strong>Tip:</strong> Sharing delivery locations helps our community see the real impact of their donations and builds trust among neighbors helping neighbors.
                        </p>
                    </div>
                    
                    <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb; text-align: center;">
                        <p style="color: #9ca3af; font-size: 14px; margin: 0;">
                            💚 Thank you for being part of Austin's caring community!
                            <br>
                            <strong>The Fast Help Team</strong>
                        </p>
                    </div>
                </div>
            </div>
